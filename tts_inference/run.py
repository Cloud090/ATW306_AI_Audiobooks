...
# Check if model is local available locally or needs to be pulled from HF.
def resolve_checkpoint(local_path: str, hf_repo: str):
    """
    If the local model folder exists and contains model files,
    return the local path. Otherwise return the HuggingFace repo ID.
    """
...
# ========================= Helpers =========================
# Rough sentence splitter, used if prompts.json has a single paragraph.
...
def load_json_prompts(path: str):
    ...
    Supports:
      ...
      2) [{"text": "..."} , ...]
      3) {"sentences": ["...", "..."]}
      4) {"paragraph": "long string"}  -> auto split into sentences
    """
    ...
    if isinstance(data, dict):
        if "sentences" in data:
            return [str(x).strip() for x in data["sentences"] if str(x).strip()]
        if "paragraph" in data:
            return split_sentences(data["paragraph"])
...
tokenizer = AutoTokenizer.from_pretrained(
    CKPT,          # expects tokeniser files in same folder as model
...
# ========================= Audio utils =========================
# Run the model and decode the speech tokens with SNAC.
# Returns a 1D numpy array (float32) at 24 kHz.
...
def clean_audio(audio, sr=24000):
    ...
    audio = nr.reduce_noise(
        ...
        y_noise=noise_clip,
        sr=sr,
        prop_decrease=0.85, # Anything higher than ~0.9 starts to degrade quality
        stationary=False,
